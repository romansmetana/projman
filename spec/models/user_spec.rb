...
RSpec.describe User, type: :model do
  ...
  describe "|| when user was deleted" do
    it "# User.count = 0" do
      expect { User.destroy_all }.to change { User.count} .from(1).to(0)
    end
    it "# all projects are deleted " do
      expect { User.destroy_all }.to change { user.projects.count}.from(1).to(0)
    end

    it "# all tasks are deleted " do
      expect { User.destroy_all }.to change { user.tasks.count}.from(1).to(0)
    end
    
    it "# all tags are deleted " do
      expect { User.destroy_all }.to change { user.tags.count}.from(1).to(0)
    end
  end
